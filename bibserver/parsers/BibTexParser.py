...
#            instring = string.replace(instring, k, self.string_dict[k])
        instring = unicode(instring,chardet.detect(instring)["encoding"],'ignore')
        if instring.find("\\") or instring.find("{"):
            for k, v in self.unicode_to_latex.iteritems():
                if instring.find(v) != -1:
                    parts = instring.split(str(v))
                    instring = k.join(parts)
                instring = instring.replace("{","").replace("}","")
        return unicode(instring)
